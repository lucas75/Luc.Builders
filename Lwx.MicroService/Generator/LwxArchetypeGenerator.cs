...
public class LwxArchetypeGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateLwxConfigure(SourceProductionContext spc, AttributeData? swaggerAttr, string ns)
    {
        ...
        var hasSwagger = swaggerAttr != null && publishLiteral != "Lwx.MicroService.Atributes.LwxStage.None";
        var swaggerServicesCode = hasSwagger ? $$"""
            var __lwxPublish = {{publishLiteral}};
            if (__lwxPublish != Lwx.MicroService.Atributes.LwxStage.None)
            {
                var shouldAdd = builder.Environment.IsDevelopment() ?
                    (__lwxPublish == Lwx.MicroService.Atributes.LwxStage.Development || __lwxPublish == Lwx.MicroService.Atributes.LwxStage.Production) :
                    (__lwxPublish == Lwx.MicroService.Atributes.LwxStage.Production);
