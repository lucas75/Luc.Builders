...
)
{
    ...
    private void GenerateSourceFiles(string name, string ns, string rootNs, string endpointClassName, string? optionalFirstSegment, string? uriArg, string? securityProfile, string? summary, string? description, string? publishLiteral)
    {
        ...
        else
        {
            ...
            sb.AppendLine(usingsBlock);
            sb.AppendLine();

            // (marker class removed) — generate only the endpoint partial that contains Configure

            // Declare the Configure method inside the original declared type's namespace & class
            sb.AppendLine($"namespace {ns}");
            sb.AppendLine("{");
            sb.AppendLine($"    public static partial class {name}");
            sb.AppendLine("    {");
            sb.AppendLine(configureMethod.FixIndent(2));
            sb.AppendLine("    }");
            sb.AppendLine("}");
            ...
            GeneratorHelpers.AddGeneratedFile(ctx, generatedFileName, sb.ToString());
    }
}
