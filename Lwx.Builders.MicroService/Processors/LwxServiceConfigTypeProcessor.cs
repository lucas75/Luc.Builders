...
{
    public void Execute(Generator parent)
    {
        ...
        // Generate Lwx endpoint extensions (swagger wiring + validation) using the attribute args
        GenerateLwxEndpointExtensions(ctx, attr.AttributeData, ns);

        // If GenerateMain was requested, generate Main now. The GenerateMain helper will validate that no user
        // Program.cs exists using the provided compilation context. Use the parent lists for endpoints and workers.
        if (generateMain)
        {
            var epList = parent.EndpointNames.ToList();
            var wkList = parent.WorkerNames.ToList();

            // GenerateMain will report diagnostics and emit the ServiceConfig.Main.g.cs file.
            _ = GenerateMain(ctx, ns, epList, wkList, compilation, loc ?? Location.None);
        }
    }
    ...
    /// </summary>
    public static void GenerateLwxEndpointExtensions(SourceProductionContext spc, AttributeData? swaggerAttr, string ns)
    {
        string title = "API";
        ...
        spc.AddSource("LwxEndpointExtensions.g.cs", SourceText.From(source, System.Text.Encoding.UTF8));
    }
    ...
    public static string? GenerateMain(SourceProductionContext spc, string ns, List<string> endpointNames, List<string> workerNames, Compilation compilation, Location serviceConfigLocation)
    {
        ...
        spc.AddSource("ServiceConfig.Main.g.cs", SourceText.From(source, System.Text.Encoding.UTF8));
        ...
        spc.ReportDiagnostic(Diagnostic.Create(
            new DiagnosticDescriptor(
                ...
                isEnabledByDefault: true),
            serviceConfigLocation, generatedName, ns, methodPreview));
